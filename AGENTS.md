...
- **Admin Operations Desk redesign — Lönar-style, BUILT & verified (2026-07-25):** admin scoped to a
  **cool neutral palette** (`.admin-scope` in globals.css — white main, grey sidebar `#f7f8fa`,
  ...
  Nav active-state matches the exact path or a sub-route (`href` or `href + "/"`) so `/admin/kings`
  no longer lights up on `/admin/kingshour`.
...
- Build order step 7 — **Email page** (segmented sends with preview/count confirmation and history),
  plus finishing **Analytics** once funnel events are persisted rather than inferred.
