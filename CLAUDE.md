...
- **Final handoff + duplicate-registration wording (2026-07-22):** tightened only the narrow-screen
  ...
  The completed-King state now has one action only—**“See what happens next.”** It no longer presents
  another email or a Back route as an alternative registration path.
...
- No `psql`/`supabase` CLI is installed locally; schema changes are currently applied through the
  Supabase dashboard. Live persistence, Resend delivery and admin auth are configured.
