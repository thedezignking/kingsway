# CLAUDE.md — Kingsway build guide

Guidance for Claude Code (and any AI agent) working in this repo. **Keep this file
...
CLAUDE.md, refreshed *in the same commit* to reflect that commit's work. This is the auto-update
...
- DB schema [supabase/migrations/0001_init.sql](./supabase/migrations/0001_init.sql) — all §6 entities,
  enums, indexes, RLS enabled (stub policies). ⚠️ Not yet run against a live Postgres/Supabase.
...
- **To actually run the loop with real data**: drop live Supabase + Resend keys into `.env.local` and
  apply `supabase/migrations/0001_init.sql` to the Supabase project.

**Known gaps / cautions**
- No `psql`/`supabase` CLI installed locally — schema hasn't been executed anywhere; the loop has
  been verified against guarded stubs (no live persistence yet).
...
- Git is intentionally untouched. Note: the git root is `C:\Users\HP` (home dir = a `velora-atelier`
  repo); this project is **not** in its own repo.
